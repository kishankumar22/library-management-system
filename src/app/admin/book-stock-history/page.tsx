...
import {
  faSpinner, faPlus, faBook, faTimes, faTimesCircle, faSearch,
...
const BookStockHistoryPage = () => {
  ...
  const [publicationSearch, setPublicationSearch] = useState('Filter by Publication');
  ...
  const handlePublicationChange = (pubId: number) => {
    ...
    setPublicationFilter(pubId); // Update filter state
    fetchData(); // Refetch data with new filter
  };
  ...
  const handleAddBookCopy = async () => {
    if (isSubmitting || !formData.BookId || formData.CopiesAdded <= 0) {
      toast.error('Please fill all required fields with valid copies');
      return;
    }
    setIsSubmitting(true);
    try {
      await axios.post('/api/book-stock-history', formData);
      toast.success('Book copies added successfully');
      setIsModalOpen(false);
      resetFormFields();
      await fetchData();
    } catch (error) {
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-4 gap-2 bg-gradient-to-r from-blue-50 to-white p-2 rounded-md">
        ...
        >
          <FontAwesomeIcon icon={faPlus} size="xs" /> Add Book Copy
        </button>
      </div>
          ...
          <div className="relative">
            <div
              ...
              onClick={() => {
                ...
                if (publicationInputRef.current) {
                  publicationInputRef.current.focus();
                  if (publicationSearch === 'Filter by Publication') {
                    setPublicationSearch(''); // Clear placeholder when clicked
                  }
                }
              }}
            >
              <input
                ...
                onChange={(e) => {
                  ...
                  setShowPublicationInput(true); // Keep dropdown open while typing
                }}
                onBlur={() => {
                  if (!publicationSearch) {
                    setPublicationSearch('Filter by Publication');
                  }
                  setTimeout(() => setShowPublicationInput(false), 200); // Delay to allow selection
                }}
              />
              {publicationSearch && publicationSearch !== 'Filter by Publication' && (
                <FontAwesomeIcon
                  icon={faTimesCircle}
                  size="xs"
                    ...
                    publications
                      ...
                      .map(pub => (
                        <li
                          ...
                          onClick={() => {
                            setPublicationSearch(pub.Name); // Set the selected publication name
                            setPublicationFilter(pub.PubId); // Update filter state
                            handleFilterChange(); // Refetch data with new filter
                            setShowPublicationInput(false); // Close dropdown
                          }}
                        >
                          {pub.Name}
                        </li>
          ...
          <table className="min-w-full divide-y divide-gray-200 text-sm">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-3 py-2 text-left text-blue-800">Copies Added</th>
                ...
                <th className="px-3 py-2 text-left text-blue-800">Modified On</th>
                <th className="px-3 py-2 text-left text-blue-800">Modified By</th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredHistory.length === 0 ? (
                <tr>
                  <td colSpan={9} className="px-3 py-2 text-center text-gray-500">No history found</td>
                </tr>
              ) : (
                filteredHistory.map((history, index) => (
                  <tr key={history.BookStockHistoryId} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2">{history.CopiesAdded}</td>
                    ...
                    <td className="px-3 py-2">{history.ModifiedOn ? new Date(history.ModifiedOn).toLocaleDateString() : 'N/A'}</td>
                    <td className="px-3 py-2">{history.ModifiedBy || 'N/A'}</td>
                  </tr>
                ))
              )}
            </tbody>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-2 z-50">
          <div ref={modalRef} className="bg-white rounded shadow w-full max-w-2xl max-h-[90vh] overflow-y-auto relative">
            ...
            <div className="p-4">
              <h2 className="text-lg font-bold mb-3 p-2 rounded-md bg-gradient-to-r from-blue-50 to-white text-blue-800">Add Book Copy</h2>
              <div className="space-y-4">
                ...
                <div className="bg-gray-50 p-3 rounded">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                      <label className="block text-xs font-medium mb-1 text-gray-700">Copies Added</label>
                      <input
                        type="number"
                        min="1"
